...
chmod +x ./unfurl
```
After that you can just run the executable in a terminal
...
in case you're not near your computer at the moment. The `-explain` flags have syntax
highlighting (which this readme does not have), so I definitely recommend using them.
...
There is of course more than one way to do this, but the technique laid out here
approaches optimal.
...
state table, but these left-moves place a lot less strain on the mind,
